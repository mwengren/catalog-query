...
def obtain_owner_org(api_url, org_name, logger=None):
    ...
    if logger: logger.info("Executing {action}.  URL: {url}. Parameters {params}".format(action=action, url=url, params=payload))
...
def package_search(api_url, org_id, start_index, logger=None, out=None):
    ...
    #print json.dumps(result, indent=4, sort_keys=True)
    if out: out.write(json.dumps(result, indent=4, sort_keys=True, ensure_ascii=False))
